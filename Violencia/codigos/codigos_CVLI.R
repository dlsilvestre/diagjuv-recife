...
func.ano(cvli_jovem, "CVLI")
func.ano(est_jovem, "Estupros")
...
func.maniB <- function(data){
        ...
        dataDem$BAIRRO <- stri_trans_general(dataDem$BAIRRO, "Latin-ASCII")
        cvli_data <- merge(dataDem, cvli_data, by = "BAIRRO", all = T)
        # cvli jov e total por bairro
        data_bairro <- data.frame(table(data$localidade, data$jovem))
        # from wide to long 
        ...
        # renomear colunas
        colnames(data_bairro) <- c("localidade", "cvli_n_jovem", "cvli_jovem", "cvli_total")
        # mergir co cvli_bairro_jovem
        data_bairro <- merge(dataDem, data_bairro, by = "localidade")
        return(data_bairro)
}
